...
def sub_add_PAIS(table_name, processor: MODEL.TableProcessor):
    centros_df = processor.GetTables('CENTROS')

    processor.AddColumns(
        table_name,
...
def sub_add_CENTROS(table_name, processor: MODEL.TableProcessor):
    centros_df = processor.GetTables('CENTROS')
...
def sub_add_CANAL(table_name, processor: MODEL.TableProcessor):
    ...
    df['CANAL'] = df.apply( lambda row: 
        "WHOLESALE DEFERRET" if str(row['Canal distribución']) == "20" and row['CENTROS'] == "G601"
        else row['CANAL'],
        axis=1
    )
...
def sub_add_CLIENTE_DESCRPCION(table_name, processor: MODEL.TableProcessor):
...
def sub_add_CLASIFICACION(table_name, processor: MODEL.TableProcessor):
...
def sub_add_FAMILIA(table_name, processor: MODEL.TableProcessor):
...
def sub_add_LINEA(table_name, processor: MODEL.TableProcessor):
...
def sub_add_MATERIAL(table_name, processor: MODEL.TableProcessor):
    ...
    df = processor.AddColumns(
        ...
        source_tables={
            'CODIGOS_CAMBIAN': codigosCambian_df
        },
    )
...
def sub_add_SEGMENTO(table_name, processor: MODEL.TableProcessor):
    ...
    df = processor.AddColumns(
        ...
        source_tables={'SEGMENTO_CLIENTE': segmento1_df},
    )
    ...
    df = processor.AddColumns(
        ...
        source_tables={'SEGMENTO_CODIGO': segmento2_df},
    )

    df['SEGMENTO'] = df.apply(
        lambda row: (
            row['SEG1'] if pd.notna(row['SEG1']) else
            row['SEG2'] if pd.notna(row['SEG2']) else
            row['CLASIFICACION']
        ),
        axis=1
    )

    df.drop(columns=['SEG1', 'SEG2'], inplace=True)
    df.to_sql(table_name, processor.conn, if_exists='replace', index=False)

def sub_add_DESCRIPTION(table_name, processor: MODEL.TableProcessor):
...
def sub_add_VALIDACION_COD(table_name, processor: MODEL.TableProcessor):
    processor.AddColumns(
    table_name,
    column_definitions={
...
def sub_add_VOLUMEN(table_name, processor: MODEL.TableProcessor):
    mara_df = processor.GetTables('MARA')
...
def sub_add_UNIDADES(table_name, processor: MODEL.TableProcessor):
...
def sub_add_MONTO_USD(table_name, processor: MODEL.TableProcessor):
    processor.AddColumns(
        table_name,
        column_definitions={
...
def sub_add_GALONES(table_name, processor: MODEL.TableProcessor):
    processor.AddColumns(
        table_name,
        column_definitions={
            'GALONES': lambda row: (
...
def sub_add_FILTRO1(table_name, processor: MODEL.TableProcessor):
    processor.AddColumns(
        table_name,
        column_definitions={
            'FILTRO1': lambda row: (
...
def sub_add_FILTRO2(table_name, processor: MODEL.TableProcessor):
...
def sub_add_FILTRO3(table_name, processor: MODEL.TableProcessor):
...
def _completarVentaHistorica(processor: MODEL.TableProcessor):
    ...
    for label, func in tqdm(steps, desc="Completando columnas", unit="columna"):
        ...
        func(table_name, processor)
        print(f"\r✔ Completado: {label}")
        print("\r------------------------------------------------")
        time.sleep(0.2)
...
def _pivotearDescargar_VENTA_BRUTA(processor: MODEL.TableProcessor):
    ...
    processor.SetTable('pivot_result', pivot_df)
    print(pivot_df)
